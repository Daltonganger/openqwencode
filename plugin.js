...
async function refreshCredentialsSingleFlight(currentCreds, { forceRefresh = false, signal } = {}) {
  if (!inFlightRefreshPromise) {
    const refreshPromise = withCredentialLock(async () => {
      let latest = await loadCredentialsFromDisk({ force: true });
      if (!latest) latest = currentCreds;

      if (!latest) return null;
      if (!forceRefresh && isCredentialFresh(latest)) return latest;

      const refreshedElsewhere = forceRefresh && latest.accessToken && didCredentialsChange(currentCreds, latest);
      if (refreshedElsewhere) return latest;

      if (!latest.refreshToken) {
        return !forceRefresh && latest.accessToken ? latest : null;
      }

      const refreshSignal = undefined;

      try {
        const refreshed = await refreshAccessTokenWithRetry(latest.refreshToken, latest.resourceUrl, refreshSignal);
    ...
    const sharedPromise = refreshPromise.finally(() => {
      if (inFlightRefreshPromise === sharedPromise) {
        inFlightRefreshPromise = null;
      }
    });

    inFlightRefreshPromise = sharedPromise;
  }

  return await waitForPromiseWithSignal(inFlightRefreshPromise, signal);
}
...
async function getValidCredentials(getAuth, { forceRefresh = false, signal } = {}) {
  ...
  return refreshCredentialsSingleFlight(creds, { forceRefresh, signal });
}
