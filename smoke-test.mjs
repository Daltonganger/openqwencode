...
test('plugin registers provider and disables legacy providers', async () => {
  ...
  const { QwenAuthPlugin } = await import(`${modulePath}?case=config-${Date.now()}`);
...
test('loader bootstraps creds, persists them, and retries with refresh on 401', async () => {
  ...
  try {
    const { QwenAuthPlugin } = await import(`${modulePath}?case=loader-${Date.now()}`);
    const plugin = await QwenAuthPlugin();
    ...
    assert.match(upstreamCalls[1].headers['x-dashscope-useragent'], /^OpenQwenCode\/0\.1\.0 /);
